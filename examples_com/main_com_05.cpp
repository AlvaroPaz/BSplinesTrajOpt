...
/**
 *	\file examples_com/main_com_05.cpp
 *	\author Alvaro Paz, Gustavo Arechavaleta
 *	\version 1.0
 *	\date 2020
 *
 *	Optimal motion generation for inertial Nao (sequential movements) -> Hands to Front
 */
...
int main(int argv, char* argc[])
{
    ...
    bound_aux.segment(19,6) = geo::VectorXr::Ones(6,1) * 0.4; //(1.3)

    optSettings->qiBound_u = bound_aux;
    optSettings->qfBound_u = bound_aux;
    optSettings->comBound_u = geo::VectorXr::Ones(2,1) * 0.026*1.2;
    optSettings->comBound_u << 0.09, 0.026;
    optSettings->muBound_u = 1e-2;

    optSettings->qiBound_l = -bound_aux;
    optSettings->qfBound_l = -bound_aux;
    optSettings->comBound_l = -geo::VectorXr::Ones(2,1) * 0.026*1.2;
    optSettings->comBound_l << -0.05, -0.026;
